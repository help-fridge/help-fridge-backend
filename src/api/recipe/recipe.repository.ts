...
export class RecipeRepository {
  ...
  > {
    return await this.prisma.$queryRaw`
      ...
      FROM recipe_food_tb rf

      LEFT JOIN (
        SELECT * FROM fridge_tb
        WHERE expired_at <= CURRENT_DATE + INTERVAL '3 days'
        AND user_idx = 1
      ) f ON rf.food_id = f.food_id

      LEFT JOIN fridge_tb fa
        ON rf.food_id = fa.food_id AND fa.user_idx = 1

      JOIN recipe_tb r
        ON rf.recipe_id = r.id

      GROUP BY rf.recipe_id, r.name

      HAVING COUNT(DISTINCT f.food_id) > 0

      ORDER BY 
        COUNT(DISTINCT f.food_id) DESC,
        COUNT(DISTINCT fa.food_id) DESC
    `;
  }
}
